...
for i = 1:nsamples
    ...
    glcmMtarix = graycomatrix(slice,'Offset',[0 1; -1 1; -1 0; -1 -1],'Symmetric', true);
    meanMatrix = glcmMtarix(:,:,1) + glcmMtarix(:,:,2) + glcmMtarix(:,:,3) + glcmMtarix(:,:,4)
    g = graycoprops(meanMatrix);
    aux = [g.Contrast, g.Correlation, g.Energy, g.Homogeneity]
    samples(i, 2) = {aux}  
    x0 = randi([1 ssize]);
    y0 = randi([1 ssize]);
end

%change depending on the class 
